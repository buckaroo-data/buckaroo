...
class LoadCompareHandler(tornado.web.RequestHandler):
    """POST /load_compare — load two files and render a styled diff table."""
    ...
    def _load_file(self, path: str):
        """Load a single file, writing an error response on failure."""
        try:
    ...
    def _push_state_to_clients(self, session):
        if not session.ws_clients:
            return
        push_msg = json.dumps(build_state_message(session, metadata=session.metadata))
        for client in list(session.ws_clients):
            try:
                client.write_message(push_msg)
            except Exception:
    ...
    async def post(self):
        ...
        # Use full Buckaroo state so compare views include summary stats/histograms.
        dataflow = create_dataflow(merged_df)
        buckaroo_state = get_buckaroo_display_state(dataflow)

        # Apply compare-specific styling overrides.
        orig_to_renamed = dict(old_col_new_col(merged_df))
        column_config = buckaroo_state["df_display_args"]["main"]["df_viewer_config"]["column_config"]
        ...
        self._push_state_to_clients(session)
