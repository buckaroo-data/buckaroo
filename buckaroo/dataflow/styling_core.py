...
def rewrite_override_col_references(
    rewrites: Dict[ColIdentifier, ColIdentifier],
    override:PartialColConfig) -> PartialColConfig:
...
class StylingAnalysis(ColAnalysis):
    ...
    @classmethod
    def fix_column_config(
        cls,
        col: ColIdentifier,
        orig_col_name: ColIdentifier,
        base_cc:BaseColumnConfig) -> ColumnConfig:
    ...
    def style_columns(cls, sd:SDType, df:pd.DataFrame) -> List[ColumnConfig]:
        ...
        for col, col_meta in sd.items():
            ...
            if 'column_config_override' in col_meta:
                ...
                base_style.update(rewrite_override_col_references(
                    rewrites,
                    cco)) # pyright: ignore[reportCallIssue, reportArgumentType]
