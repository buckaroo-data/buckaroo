...
def prepare_df_for_serialization(df:pl.DataFrame) -> pl.DataFrame:
    ...
    select_clauses = [col_alias(
        old,
        new) for old, new in old_col_new_col(df.select(pl.exclude('index'))) if not old == "index"]
    ...
    def _handle_payload_args(self, new_payload_args):
        ...
        try:
            ...
            if sort:
                ...
                self.send(
                    { "type": "infinite_resp", 'key':new_payload_args, 'data':[], 'length':len(processed_df)},
                    [to_parquet(slice_df)])
            else:
