...
class LazyInfinitePolarsBuckarooWidget(anywidget.AnyWidget):
    ...
    def _add_message(self, msg_type: str, message: str, show_message_box: bool, **kwargs) -> None:
        ...
        msg_box_enabled = show_message_box or (self.show_message_box.get(
            'enabled',
            False) if isinstance(self.show_message_box, dict) else False)
    ...
    def _load_and_filter_cached_data(
        self,
        all_cols: List[str],
        show_message_box: bool) -> Optional[Dict[str, Dict[str, Any]]]:
        """
    ...
    def _log_execution_update(self, progress_note, show_message_box: bool) -> None:
        ...
        num_expressions = len(progress_note.execution_args.expressions) if hasattr(
            progress_note.execution_args,
    ...
    def ensure_df_display_args(
        ...
        def _schema_column_config() -> list[dict[str, object]]:
            return [{
                ...
                "header_name": c,
                "displayer_args": {"displayer": "obj"}} for c in all_cols]
        ...
        logger.info(
            "LazyInfinite init: total_rows=%s; initial columns=%s",
            self.df_meta['total_rows'],
    ...
    def __init__(
        ...
        has_status = any(
            ...
            if isinstance(col_stats, dict))
        logger.info(f"LazyInfinitePolarsBuckarooWidget.__init__: __status__ present: {has_status}, show_message_box trait: {self.show_message_box}, message_log messages count: {len(self.message_log.get(
            'messages',
            []))}")
    ...
    def _handle_payload_args(self, new_payload_args: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.send(
                {"type": "infinite_resp", 'key': new_payload_args, 'data': [], 'length': self.df_meta['total_rows']},
            ...
            if second_pa:
                ...
                if s2 is not None and e2 is not None:
                    ...
                    self.send(
                        {"type": "infinite_resp", 'key': second_pa, 'data': [], 'length': self.df_meta['total_rows']},
                        [self._to_parquet(slice2)])
        except Exception as e:
            stack_trace = traceback.format_exc()
            self.send(
                {"type": "infinite_resp", 'key': new_payload_args, 'data': [], 'error_info': stack_trace, 'length': 0},
                [])
            logger.exception("error handling payload args: %s", e)

    def add_analysis(
        self,
        analysis_klass: Type[PolarsAnalysis],
        ...
        if pinned_row_configs:
            ...
            current_pinned_rows = current_config.get(
                'pinned_rows',
                []).copy() if current_config.get('pinned_rows') else []
