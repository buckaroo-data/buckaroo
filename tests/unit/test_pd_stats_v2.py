...
class TestTypeComputed:
    ...
    def test_period(self):
        assert (
            self._run(pd.Series(pd.period_range("2021-01", periods=3, freq="M")))
            == "period"
        )

    def test_interval(self):
        assert (
            self._run(pd.Series(pd.arrays.IntervalArray.from_breaks([0, 1, 2])))
            == "interval"
        )
...
class TestFullPipeline:
    ...
    def test_mixed_df(self):
        ...
        by_orig = {
            col_stats["orig_col_name"]: col_stats for col_stats in result.values()
        }
