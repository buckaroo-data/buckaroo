...
class SimpleColumnExecutor(ColumnExecutor[ExecutorArgs]):
    def get_execution_args(self, existing_stats:dict[str,dict[str,object]]) -> ExecutorArgs:
        ...
        return ExecutorArgs(columns=columns, column_specific_expressions=False, include_hash=include_hash,
            expressions=expressions, row_start=None, row_end=None, extra=None)
...
class PriorsAwareFailOnSumExecutor(SimpleColumnExecutor):
    ...
    def get_execution_args(self, existing_stats:dict[str,dict[str,object]]) -> ExecutorArgs:
        ...
        exprs: list[pl.Expr] = [pl.all().pl_series_hash.hash_xx().name.suffix("_hash"),
        ...
        return ExecutorArgs(columns=columns, column_specific_expressions=False, include_hash=True, expressions=exprs,
            row_start=None, row_end=None, extra=None)
...
def test_bisector_on_success_event_noop():
    ...
    success_input = ExecutorLogEvent(dfi=exc.dfi, args=starting_args, start_time=dtdt.now(), end_time=dtdt.now(),
        completed=True)
...
def test_column_bisector():
    ...
    existing_stats = {'a1':{}, 'b2':{}}
    starting_args = SimpleColumnExecutor().get_execution_args(existing_stats)  # type: ignore
    starting_ev = ExecutorLogEvent(dfi=exc.dfi, args=starting_args, start_time=dtdt.now(), end_time=None,
        completed=False)
...
def test_column_bisector_with_priors_provider():
    ...
    start_ev = ExecutorLogEvent(dfi=(id(ldf), ''), args=start_args, start_time=dtdt.now(), end_time=None,
        completed=False)
...
def test_column_bisector_on_success_event_noop():
    ...
    existing_stats = {'a1':{}, 'b2':{}}
    starting_args = SimpleColumnExecutor().get_execution_args(existing_stats)  # type: ignore
    starting_ev = ExecutorLogEvent(dfi=exc.dfi, args=starting_args, start_time=dtdt.now(), end_time=dtdt.now(),
        completed=True)
...
def test_row_range_bisector_minimal_and_success():
    ...
    existing_stats = {'a1':{}, 'b2':{}}
    starting_args = SimpleColumnExecutor().get_execution_args(existing_stats)  # type: ignore
    starting_ev = ExecutorLogEvent(dfi=(id(ldf2), ''), args=starting_args, start_time=dtdt.now(), end_time=None,
        completed=False)
...
def test_row_range_bisector_on_success_event_noop():
    ...
    existing_stats = {'a1':{}, 'b2':{}}
    starting_args = SimpleColumnExecutor().get_execution_args(existing_stats)  # type: ignore
    success_ev = ExecutorLogEvent(dfi=(id(ldf2), ''), args=starting_args, start_time=dtdt.now(), end_time=dtdt.now(),
        completed=True)
...
def test_full_bisect_pipeline_sum_and_rows():
    ...
    N = 50
    df2 = pl.DataFrame({'a1': list(range(N)), 'b2': list(range(N, 2*N)), 'c3': [i * 0.5 for i in range(N)],
        'd4': [i % 7 for i in range(N)], 's1': [f"s{i}" for i in range(N)], 's2': [f"t{i%3}" for i in range(N)]})
    ldf2 = df2.lazy()
...
def test_full_bisect_pipeline_row_dependent():
    ...
    N = 100
    df2 = pl.DataFrame({'a1': list(range(N)), 'b2': list(range(N, 2*N)), 'c3': [i * 0.25 for i in range(N)],
        'd4': [i % 5 for i in range(N)], 's1': [f"s{i}" for i in range(N)], 's2': [f"t{i%4}" for i in range(N)]})
    ldf2 = df2.lazy()
