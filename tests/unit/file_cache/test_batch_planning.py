    ...
    context = PlanningContext(
        ...
        execution_history=[
            ExecutionResult(
                columns=['a', 'b', 'c', 'd'],
                success=False,
                execution_time=timedelta(seconds=30.0),
                timed_out=True),  # Half batch timeout
            ExecutionResult(
                columns=['a'],
                success=True,
                execution_time=timedelta(seconds=2.0),
                timed_out=False)  # Single column success
        ],
    ...
    context = PlanningContext(
        ...
        execution_history=[
            ...
            ExecutionResult(
                ...
                success=True,
                execution_time=timedelta(seconds=2.0),
                timed_out=False),  # 1 column success
            ExecutionResult(
                columns=['b', 'c'],
                success=True,
                execution_time=timedelta(seconds=3.0),
                timed_out=False)  # 2 columns success
        ],
    ...
    context = PlanningContext(
        ...
        execution_history=[
            ...
            ExecutionResult(
                ...
                success=False,
                execution_time=timedelta(seconds=30.0),
                timed_out=True)  # 2 columns failed
        ],
