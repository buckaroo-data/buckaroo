...
def test_index_styling3():
    assert [{'col_path':['index_name_1'], 'field':'index_a', 'displayer_args': {'displayer': 'obj'}},
        {
            'col_path':['index_name_2'],
            'field':'index_b',
            'displayer_args': {'displayer': 'obj'}}] == StylingAnalysis.get_left_col_configs(get_multiindex_with_names_index_df())
    ...
    left_col_configs: List[ColumnConfig] = [
                {
                    ...
                    "displayer_args": {
                        "displayer": "obj"}},
        {"col_path": ["level_a", "level_b"], "field": "index_b", "displayer_args": {
                        "displayer": "obj"}}]
    ...
    assert [{'col_path':['', '', 'index_name_1'], 'field':'index_a', 'displayer_args': {'displayer': 'obj'}},
        {
            'col_path':['', '', 'index_name_2'],
            'field':'index_b',
            'displayer_args': {'displayer': 'obj'}}] == StylingAnalysis.get_left_col_configs(get_multiindex_index_with_names_multiindex_cols_df())

def test_index_styling6():
    assert [{'col_path':['', '', 'index_name_1'], 'field':'index_a', 'displayer_args': {'displayer': 'obj'}},
        {
            'col_path':['level_a', 'level_b', 'index_name_2'],
            'field':'index_b',
            'displayer_args': {'displayer': 'obj'}}] == StylingAnalysis.get_left_col_configs(get_multiindex_with_names_both())
...
def test_get_dfviewer_column_config_override():
    ...
    expected_output: DFViewerConfig = {'pinned_rows': [], 'column_config':  [
            {'col_name': 'a', 'header_name':'sent_int_col', 'displayer_args': {'displayer': 'obj'}},
        b_config,
        {'col_name': 'c',
            'displayer_args': {'displayer': 'obj'},
            'header_name': 'Volume_colors'}], 'left_col_configs': [{'col_name': 'index', 'header_name':'index',
